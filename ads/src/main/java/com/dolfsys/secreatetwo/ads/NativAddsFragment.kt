...
import com.dolfsys.secreatetwo.ads.domain.models.AdsItem
import com.dolfsys.secreatetwo.ads.domain.models.Data
import com.dolfsys.secreatetwo.ads.utils.AdsEvent
import com.dolfsys.secreatetwo.ads.utils.Response
import com.dolfsys.secreatetwo.ads.utils.SavedAds
...
abstract class NativAddsFragment:DialogFragment() {

    abstract val nativeAdLayout : LinearLayout?
    ...
    fun loadUnityBannerAd(){
        adContainer?.let {
            ...
            UnityAds.initialize(requireActivity().applicationContext, unityAds?.adAppKey, false)
            ...
            val unityAdview = BannerView(requireActivity(),unityAds?.bannerAdKey, UnityBannerSize(320,50))
            unityAdview.load()
    ...
    fun loadMediationUnityInterstitialAd(){
        unityAds?.let {unityads ->
            ...
            UnityMediationinterstitialAd =
                unityads.interstitialAdKey?.let { com.unity3d.mediation.InterstitialAd(requireActivity(), "interstitialAdUnitId") }
    ...
    private fun initUnityintestitalAds(){
        unityAds?.let {unityads ->
            UnityAds.initialize(requireActivity(),unityads.adAppKey,false,object :IUnityAdsInitializationListener{
    ...
    private fun showUnityinterstitalAds(){
        ...
        unityAds?.let {
            UnityAds.show(requireActivity(), it.interstitialAdKey, UnityAdsShowOptions(), showListener)
        } ?: adsViewModel.onAdEvent(AdsEvent.onAdClose)
    ...
    fun loadNativeAds(){
        ...
        val nativeAd = fabBookAds?.nativeAdKey?.let {
            NativeAd(requireContext(), fabBookAds?.nativeAdKey)
        }
        val nativeAdListener = object :NativeAdListener{
            override fun onError(p0: Ad?, p1: com.facebook.ads.AdError?) {
                Log.d("nativeads", "onError: in load ${p1?.errorMessage.toString()}")
            }

            override fun onAdLoaded(p0: Ad?) {
                val adView: View = NativeAdView.render(requireContext(), nativeAd)
